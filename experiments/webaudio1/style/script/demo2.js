...
angular.module('app').factory('SoundEntity', function($rootScope, SoundService, EntityService){
	...
	var soundSources = [soundSource0, soundSource1];

	// EntityService.drawingChain.addEntity(soundSource0);
	// EntityService.drawingChain.addEntity(soundSource1);

	var sounds = ['bubidibup.mp3','bla atm.mp3', 'Impulsantwort.wav', 'effect.wav', 'effect2.wav', 'effect3.wav', 'effect4.wav', 'effect5.wav'];
	...
	SoundEntity.startMusicSamples = function(i){
		if(startedMusic[i]){
...
// okay here begins the audio related stuff
angular.module('app').factory('SoundService', function(){
	var SoundService = new Object();

	// okay here begins the AUDIO related stuff
	SoundService.context = new webkitAudioContext();
